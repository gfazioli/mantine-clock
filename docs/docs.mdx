...
The `Clock` component supports an entrance animation when it first mounts. Enable it with the `withMountAnimation` prop. You can control the animation duration with `mountAnimationDuration` (in milliseconds).
...
For full control over how hands are drawn, use the `renderHourHand`, `renderMinuteHand`, and `renderSecondHand` render props. Each receives `{ angle, length, center }` and must return an SVG element.
...
function Demo() {
  return (
    <Clock
      size={300}
      renderSecondHand={({ angle, length, center }) => (
        <line
          x1={center}
          y1={center}
...
The `size` prop accepts any valid CSS value, including responsive values via Mantine's `rem()` or viewport units. This makes it easy to build clocks that scale with the viewport.

```tsx
import { Clock } from '@gfazioli/mantine-clock';

function Demo() {
  return <Clock size="min(300px, 50vw)" />;
}
```
