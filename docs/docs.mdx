  ...
  useClockReturns: {
    props: {
      ...
      formattedHours: {
        name: 'formattedHours',
        type: { name: 'string' },
        description: 'Formatted hour string with padding if enabled',
        required: true,
        defaultValue: null,
      },
      formattedMinutes: {
        ...
        type: { name: 'string' },
        description:
          'Formatted minute string with padding if enabled',
        required: true,
        defaultValue: null,
      },
      formattedSeconds: {
        ...
        type: { name: 'string' },
        description:
          'Formatted second string with padding if enabled',
        required: true,
        defaultValue: null,
      },
      milliseconds: {
  ...
  useClockCountDownReturns: {
    props: {
      ...
      hours: {
        name: 'hours',
        type: { name: 'number' },
        description: 'The remaining hours',
        required: true,
        defaultValue: null,
      },
      minutes: {
      ...
      formattedDays: {
        name: 'formattedDays',
        type: { name: 'string' },
        description: 'Formatted days with padding',
        required: true,
        defaultValue: null,
      },
      formattedHours: {
        name: 'formattedHours',
        type: { name: 'string' },
        description: 'Formatted hours with padding',
        required: true,
        defaultValue: null,
      },
      formattedMinutes: {
        name: 'formattedMinutes',
        type: { name: 'string' },
        description: 'Formatted minutes with padding',
        required: true,
        defaultValue: null,
      },
      formattedSeconds: {
        name: 'formattedSeconds',
        type: { name: 'string' },
        description: 'Formatted seconds with padding',
        required: true,
        defaultValue: null,
      },
      pause: {
      ...
      start: {
        ...
        type: { name: 'function' },
        description:
          'Function to start the countdown (alias for resume)',
        required: true,
        defaultValue: null,
      },
      reset: {
...
By default the `Clock` renders as a circle. You can switch to a rounded-rectangle shape (Apple Watch style) using the `shape` prop. Control the corner radius with `borderRadius` and the vertical stretch with `aspectRatio`.
...
For full control over how hands are drawn, use the `renderHourHand`, `renderMinuteHand`, and `renderSecondHand` render props. Each receives `{ angle, length, centerX, centerY, width, clockSize }` and must return a React node.

<Demo data={demos.customHands} />
...
The `sectors` prop accepts an array of time ranges that are rendered as coloured arcs on the clock face. Each sector has `from` and `to` times (in `"HH:MM"` format), a `color`, and an optional `label`.

<Demo data={demos.sectorsDemo} />

## Face Content

Use the `faceContent` prop to render arbitrary React content at the centre of the clock face. This is useful for brand logos, labels, or additional information.

<Demo data={demos.faceContentDemo} />

## Mount Animation

The `Clock` component supports an entrance animation when it first mounts. Enable it with the `animateOnMount` prop. You can control the animation duration with `animateOnMountDuration` (in milliseconds).

<Demo data={demos.mountAnimation} />

## Responsive Size

The `size` prop supports responsive values via Mantine's breakpoint objects. You can pass a single value or an object with breakpoint keys to change the clock size at different screen widths:
...
<Clock size={{ base: "sm", md: 400, lg: "xl" }} />

// Auto size — fills the parent container
<Clock size="auto" />
```

`Clock.Digital` also supports responsive `size` and `gap` props using the same pattern:
...
<Demo data={demos.responsiveSize} />
...
The `onTimeChange` callback fires on every tick with the current time data. This is useful for synchronising external state or triggering side effects based on the displayed time.
...
`Clock` supports [Styles API](https://mantine.dev/styles/styles-api/), you can add styles to any inner element of the component with `classNames` prop. Follow [Styles API](https://mantine.dev/styles/styles-api/) documentation to learn more.

You can customize the appearance of the `Clock` component using the Styles API. This allows you to modify styles for various parts of the component, such as hands, tick marks, numbers, center dot, and more.
...
The `Clock.Digital` compound component renders a digital clock display. It supports timezone, 12/24-hour format, AM/PM indicator, date display, custom separators, and Mantine size tokens.

<Demo data={demos.digitalConfigurator} />

### Clock.Digital Examples

<Demo data={demos.digital} />

### Clock.Digital Styles API

<Demo data={demos.digitalStylesApi} />
